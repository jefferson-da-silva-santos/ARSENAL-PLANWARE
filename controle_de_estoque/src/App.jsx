...
import {
  Package, LayoutDashboard, ArrowUpDown, AlertTriangle, Search, Plus, X,
  Pencil, Trash2, FileText, Loader2, TrendingUp, TrendingDown, CheckCircle,
  AlertCircle, ClipboardList, RefreshCw, Menu, ArrowUp, ArrowDown,
  DollarSign, Moon, Sun, LogOut,
} from "lucide-react";
...
function setState(next) { _state = next; _listeners.forEach(fn => fn(next)); }
...
function doLogin(token, user) { auth.save(token, user); setState({ user, logged: true }); }
function doLogout()           { auth.clear();           setState({ user: null, logged: false }); }
...
function useTheme() {
  const [theme, setTheme] = useState(() => {
    try { return localStorage.getItem("stockpro-theme") || "dark"; } catch { return "dark"; }
  });

  useEffect(() => {
    document.documentElement.setAttribute("data-theme", theme);
    try { localStorage.setItem("stockpro-theme", theme); } catch (err) {console.log(err);}
...
function ThemeToggle({ theme, onToggle }) {
  return (
    <button className="theme-toggle" onClick={onToggle} title={theme === "dark" ? "Tema claro" : "Tema escuro"} aria-label="Alternar tema">
    ...
    <div className="toast-area">
      {toasts.map(t => (
        <div key={t.id} className={`toast ${t.type}`}>
          {t.type === "success" ? <CheckCircle className="toast-icon" /> : <AlertCircle className="toast-icon" />}
          {t.message}
        </div>
      ))}
    </div>
...
// ─────────────────────────────────────────────
const EMPTY_PRODUCT = {
  nome: "", sku: "", marca: "", categoria: "",
  preco_custo: "", preco_venda: "", quantidade_estoque: "",
  estoque_minimo: 5, unidade_medida: "un", ncm: "",
};
...
function ProductModal({ product, onClose, onSave, toast }) {
  ...
  const validate = () => {
    ...
    if (form.preco_venda === "" || isNaN(form.preco_venda) || Number(form.preco_venda) < 0) errs.preco_venda = "Preço inválido";
    setErrors(errs);
    ...
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        <div className="modal-header">
          <h3><Package /> {isEdit ? "Editar Produto" : "Novo Produto"}</h3>
        ...
        <div className="modal-body">
          <div className="form-grid">
            <div className="form-group form-full">
              ...
              <input className={`form-input ${errors.nome ? "error" : ""}`} value={form.nome} onChange={e => set("nome", e.target.value)} placeholder="Ex: Parafuso Phillips M4" />
              {errors.nome && <span className="form-error">{errors.nome}</span>}
            </div>
            <div className="form-group">
              <label className="form-label">SKU *</label>
              <input className={`form-input ${errors.sku ? "error" : ""}`} value={form.sku} onChange={e => set("sku", e.target.value.toUpperCase())} placeholder="Ex: PAR-M4-001" />
              {errors.sku && <span className="form-error">{errors.sku}</span>}
            </div>
            <div className="form-group">
              <label className="form-label">Marca</label>
              <input className="form-input" value={form.marca} onChange={e => set("marca", e.target.value)} placeholder="Ex: Tramontina" />
            </div>
            <div className="form-group">
              <label className="form-label">Categoria</label>
              <input className="form-input" value={form.categoria} onChange={e => set("categoria", e.target.value)} placeholder="Ex: Fixadores" />
            </div>
            <div className="form-group">
              <label className="form-label">Unidade de Medida</label>
              <select className="form-select" value={form.unidade_medida} onChange={e => set("unidade_medida", e.target.value)}>
            ...
            <div className="form-group">
              ...
              <input className="form-input" value={form.ncm} onChange={e => set("ncm", e.target.value)} placeholder="Ex: 7318.15.00" />
            </div>
            <div className="form-group">
              <label className="form-label">Preço de Custo (R$)</label>
              <input className="form-input" type="number" min="0" step="0.01" value={form.preco_custo} onChange={e => set("preco_custo", e.target.value)} placeholder="0,00" />
            </div>
            <div className="form-group">
              <label className="form-label">Preço de Venda (R$) *</label>
              <input className={`form-input ${errors.preco_venda ? "error" : ""}`} type="number" min="0" step="0.01" value={form.preco_venda} onChange={e => set("preco_venda", e.target.value)} placeholder="0,00" />
              {errors.preco_venda && <span className="form-error">{errors.preco_venda}</span>}
            </div>
            <div className="form-group">
              <label className="form-label">Qtd. Inicial em Estoque</label>
              <input className="form-input" type="number" min="0" value={form.quantidade_estoque} onChange={e => set("quantidade_estoque", e.target.value)} placeholder="0" />
            </div>
            <div className="form-group">
              <label className="form-label">Estoque Mínimo (Alerta)</label>
              <input className="form-input" type="number" min="0" value={form.estoque_minimo} onChange={e => set("estoque_minimo", e.target.value)} />
              <span className="form-hint">Alerta quando atingir este valor</span>
            </div>
          </div>
        </div>
        <div className="modal-footer">
          ...
          <button className="btn btn-primary" onClick={submit} disabled={saving}>
            {saving ? <Loader2 className="spinner" style={{ width: 15, height: 15 }} /> : <Plus />}
...
function MovementModal({ product, onClose, onSave, toast }) {
  ...
  return (
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        <div className="modal-header">
          <h3><ArrowUpDown /> Movimentação de Estoque</h3>
          <button className="modal-close" onClick={onClose}><X /></button>
        </div>
        <div className="modal-body">
          <div style={{ background: "var(--bg-elevated)", borderRadius: "var(--radius)", padding: "10px 14px", marginBottom: 20, display: "flex", alignItems: "center", gap: 10 }}>
            <div className="product-avatar">{initials(product.nome)}</div>
            <div>
              <div style={{ fontWeight: 600, fontSize: "0.9rem" }}>{product.nome}</div>
              <div style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>
                SKU: {product.sku} · Estoque atual:{" "}
                <strong style={{ color: product.quantidade_estoque <= product.estoque_minimo ? "var(--danger)" : "var(--success)" }}>
          ...
          <div className="movement-type-btns">
            <button className={`movement-type-btn ${type === "IN" ? "active-in" : ""}`} onClick={() => setType("IN")}><ArrowUp /> Entrada</button>
            <button className={`movement-type-btn ${type === "OUT" ? "active-out" : ""}`} onClick={() => setType("OUT")}><ArrowDown /> Saída</button>
          </div>
          <div className="form-grid">
            <div className="form-group">
              <label className="form-label">Quantidade *</label>
              <input className="form-input" type="number" min="1" value={quantity} onChange={e => setQty(e.target.value)} placeholder="0" autoFocus />
            </div>
            <div className="form-group">
              <label className="form-label">Motivo</label>
              <input className="form-input" value={reason} onChange={e => setReason(e.target.value)} placeholder={type === "IN" ? "Compra, Devolução..." : "Venda, Ajuste..."} />
            </div>
          </div>
        </div>
        <div className="modal-footer">
          ...
          <button className={`btn ${type === "IN" ? "btn-primary" : "btn-danger"}`} onClick={submit} disabled={saving}>
            {saving ? <Loader2 className="spinner" style={{ width: 15, height: 15 }} /> : type === "IN" ? <ArrowUp /> : <ArrowDown />}
  ...
  return (
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{ maxWidth: 420 }}>
        <div className="modal-header">
          <h3><Trash2 /> Excluir Produto</h3>
          <button className="modal-close" onClick={onClose}><X /></button>
        </div>
        <div className="delete-modal-body">
          <div className="delete-icon-wrap"><Trash2 /></div>
          <h3>Tem certeza?</h3>
          <p>O produto <strong>"{product.nome}"</strong> será desativado.</p>
        </div>
        <div className="modal-footer">
          <button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
          <button className="btn btn-danger" onClick={confirm} disabled={deleting}>
            {deleting ? <Loader2 className="spinner" style={{ width: 15, height: 15 }} /> : <Trash2 />}
            Confirmar Exclusão
          </button>
        </div>
      </div>
  ...
  return (
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal modal-wide">
        <div className="modal-header">
          <h3><ClipboardList /> Histórico — {product.nome}</h3>
          <button className="modal-close" onClick={onClose}><X /></button>
        </div>
        <div style={{ maxHeight: 420, overflowY: "auto" }}>
          {loading ? (
            <div className="loading-state"><Loader2 className="spinner" /> Carregando...</div>
          ) : movements.length === 0 ? (
            ...
            <div className="movement-list">
              {movements.map(m => (
                <div key={m.id} className="movement-item">
                  <div className={`movement-dot ${m.type === "IN" ? "in" : "out"}`}>
                    {m.type === "IN" ? <ArrowUp /> : <ArrowDown />}
                  </div>
                  <div className="movement-info">
                    <div className="movement-product">{m.type === "IN" ? "Entrada" : "Saída"}</div>
                    <div className="movement-reason">{m.reason || "Ajuste Manual"}</div>
                  </div>
                  <div className={`movement-qty ${m.type === "IN" ? "in" : "out"}`}>
...
function DashboardPage({ onNavigate }) {
  ...
  const load = useCallback(() => {
    setLoading(true);
    api("/dashboard").then(d => setData(d?.data || d)).catch(console.error).finally(() => setLoading(false));
  }, []);

  useEffect(() => { load(); }, [load]);

  if (loading) return <div className="loading-state"><Loader2 className="spinner" /> Carregando dashboard...</div>;
  if (!data) return null;

  return (
    <div>
      <div className="page-header">
        <div className="page-header-text"><h2>Dashboard</h2><p>Visão geral do inventário em tempo real</p></div>
        <button className="btn btn-secondary" onClick={load}><RefreshCw /> Atualizar</button>
      </div>

      <div className="stats-grid">
        <div className="stat-card accent">
          <div className="stat-icon accent"><Package /></div>
        ...
        <div className="stat-card success">
          ...
          <div className="stat-info">
            ...
            <span className="stat-value" style={{ fontSize: "1.2rem" }}>{fmt(data.totalValue)}</span>
            <span className="stat-sub">Preço de venda × qtd</span>
          </div>
        </div>
        <div className={`stat-card ${data.lowStock > 0 ? "danger" : "success"}`}>
          <div className={`stat-icon ${data.lowStock > 0 ? "danger" : "success"}`}>
          ...
          <div className="stat-info">
            ...
            <span className="stat-sub">{data.lowStock > 0 ? "Requerem atenção" : "Tudo em ordem"}</span>
          </div>
        </div>
        <div className="stat-card warning">
          <div className="stat-icon warning"><ArrowUpDown /></div>
          <div className="stat-info">
            <span className="stat-label">Movimentações</span>
            <span className="stat-value">{data.recentMovements?.length ?? 0}</span>
      ...
      <div className="card">
        <div className="card-header">
          <span className="card-title"><ClipboardList /> Últimas Movimentações</span>
          <button className="btn btn-ghost" style={{ fontSize: "0.78rem", padding: "6px 12px" }} onClick={() => onNavigate("movements")}>Ver todas</button>
        </div>
        {!data.recentMovements?.length ? (
          <div className="empty-state" style={{ padding: 40 }}>
            <div className="empty-icon"><ArrowUpDown /></div>
...
function MovementsPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    api("/movements").then(d => setMovements(d?.data || d || [])).catch(console.error).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div>
      <div className="page-header">
        <div className="page-header-text"><h2>Movimentações</h2><p>Histórico completo de entradas e saídas</p></div>
        <button className="btn btn-secondary" onClick={load}><RefreshCw /> Atualizar</button>
      </div>
      <div className="card">
        <div className="card-header">
          <span className="card-title"><ArrowUpDown /> Todas as Movimentações</span>
          <span className="badge badge-accent">{movements.length} registros</span>
        </div>
        {loading ? (
          <div className="loading-state"><Loader2 className="spinner" /> Carregando...</div>
        ) : movements.length === 0 ? (
          <div className="empty-state"><div className="empty-icon"><ArrowUpDown /></div><h3>Sem movimentações</h3><p>Nenhuma movimentação registrada ainda.</p></div>
        ) : (
          <div className="movement-list">
            {movements.map(m => (
              <div key={m.id} className="movement-item">
                ...
                <div className="movement-info">
                  ...
                  <div className="movement-reason">
                    <span className="sku-mono" style={{ marginRight: 6 }}>{m.sku}</span>
...
function ProductsPage({ toast }) {
  ...
  const lowCount = products.filter(p => p.quantidade_estoque <= p.estoque_minimo).length;
    ...
    <div>
      <div className="page-header">
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button className="btn btn-secondary" onClick={() => window.open(`${API_BASE}/report/pdf`, "_blank")}><FileText /> Exportar PDF</button>
          <button className="btn btn-primary" onClick={() => openModal("create")}><Plus /> Novo Produto</button>
        </div>
      </div>
      ...
      {lowCount > 0 && (
        <div className="alert-banner">
          <AlertTriangle />
          <strong>{lowCount} produto{lowCount > 1 ? "s" : ""}</strong>&nbsp;com estoque abaixo do mínimo.
        </div>
      )}

      <div className="card">
        <div className="card-header">
          <span className="card-title"><Package /> Inventário</span>
          <div className="search-wrap">
            <Search className="search-icon" />
            <input ref={searchRef} className="search-input" placeholder="Nome, SKU, marca..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        </div>
        <div className="table-wrap">
          {loading ? (
            <div className="loading-state"><Loader2 className="spinner" /> Sincronizando...</div>
          ) : products.length === 0 ? (
            <div className="empty-state">
              <div className="empty-icon"><Package /></div>
              <h3>{search ? "Nenhum resultado" : "Sem produtos"}</h3>
              <p>{search ? `Nenhum produto encontrado para "${search}"` : "Comece cadastrando seu primeiro produto."}</p>
              {!search && <button className="btn btn-primary" onClick={() => openModal("create")}><Plus /> Cadastrar Produto</button>}
            </div>
          ) : (
            <table>
              <thead>
                <tr><th>Produto</th><th>SKU</th><th>Marca</th><th>Preço Custo</th><th>Preço Venda</th><th>Estoque</th><th>Status</th><th>Ações</th></tr>
              </thead>
              <tbody>
                {products.map(p => {
                    ...
                    <tr key={p.id} className={isLow ? "row-danger" : ""}>
                      <td data-label="Produto">
                        <div className="cell-product">
                          <div className="product-avatar">{initials(p.nome)}</div>
                          <div>
                            <div className="product-name">{p.nome}</div>
                            {p.categoria && <div className="product-cat">{p.categoria}</div>}
                          </div>
                        </div>
                      </td>
                      <td data-label="SKU"><span className="sku-mono">{p.sku}</span></td>
                      <td data-label="Marca">{p.marca || <span style={{ color: "var(--text-muted)" }}>—</span>}</td>
                      <td data-label="Preço Custo" className="price-cell">{fmt(p.preco_custo)}</td>
                      <td data-label="Preço Venda" className="price-cell">{fmt(p.preco_venda)}</td>
                      <td data-label="Estoque">
                        <span className={`stock-num ${isLow ? "low" : "ok"}`}>
                          {p.quantidade_estoque} <span style={{ fontSize: "0.7rem", fontWeight: 400, color: "var(--text-muted)" }}>{p.unidade_medida}</span>
                        </span>
                      </td>
                      <td data-label="Status">
                        {isLow
                          ? <span className="badge badge-danger"><AlertTriangle /> Baixo</span>
                          : <span className="badge badge-success"><CheckCircle /> Normal</span>}
                      </td>
                      <td data-label="Ações">
                        <div className="cell-actions">
                          <button className="btn btn-success-icon" title="Entrada/Saída" onClick={() => openModal("movement", p)}><ArrowUpDown style={{ width: 13, height: 13 }} /></button>
                          <button className="btn btn-icon-sm" title="Histórico" onClick={() => openModal("history", p)}><ClipboardList style={{ width: 13, height: 13 }} /></button>
                          <button className="btn btn-icon-sm" title="Editar" onClick={() => openModal("edit", p)}><Pencil style={{ width: 13, height: 13 }} /></button>
                          <button className="btn btn-danger-icon" title="Excluir" onClick={() => openModal("delete", p)}><Trash2 style={{ width: 13, height: 13 }} /></button>
                        </div>
                      </td>
                    </tr>
                  );
      ...
      {(modal === "create" || modal === "edit") && <ProductModal product={selected} onClose={closeModal} onSave={handleSave} toast={toast} />}
      {modal === "movement" && selected && <MovementModal product={selected} onClose={closeModal} onSave={handleSave} toast={toast} />}
      {modal === "delete"   && selected && <DeleteModal   product={selected} onClose={closeModal} onConfirm={handleSave} toast={toast} />}
      {modal === "history"  && selected && <HistoryModal  product={selected} onClose={closeModal} />}
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    if (!logged) return;
    api("/alerts").then(d => setLowCount((d?.data || d || []).length)).catch(() => {});
  }, [page, logged]);

  const navigate = p => { setPage(p); setSidebarOpen(false); };

  const pages = { dashboard: "Dashboard", products: "Produtos", movements: "Movimentações" };
    ...
    <div className="app">
      ...
      <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}>
        <div className="sidebar-logo">
          <div className="logo-icon"><Package /></div>
        ...
        <nav className="sidebar-nav">
          ...
          <button className={`nav-item ${page === "dashboard" ? "active" : ""}`} onClick={() => navigate("dashboard")}><LayoutDashboard /> Dashboard</button>
          <button className={`nav-item ${page === "products" ? "active" : ""}`} onClick={() => navigate("products")}>
            <Package /> Produtos {lowCount > 0 && <span className="nav-badge">{lowCount}</span>}
          </button>
          <button className={`nav-item ${page === "movements" ? "active" : ""}`} onClick={() => navigate("movements")}><ArrowUpDown /> Movimentações</button>
          <span className="nav-section-label" style={{ marginTop: 12 }}>Relatórios</span>
          <button className="nav-item" onClick={() => window.open(`${API_BASE}/report/pdf`, "_blank")}><FileText /> Exportar PDF</button>
        </nav>
        ...
        <div className="sidebar-footer">
          {user && (
            <div className="sidebar-user">
              ...
              <button className="sidebar-logout-btn" onClick={() => { doLogout(); toast("Sessão encerrada", "error"); }} title="Sair">
      ...
      <div className="main">
        <header className="topbar">
          <div className="topbar-left">
            <button className="btn btn-ghost" style={{ display: "none", padding: "6px", width: 34, height: 34 }} id="menu-btn" onClick={() => setSidebarOpen(true)}>
          ...
          <div className="topbar-right">
            {lowCount > 0 && (
              <div style={{ display: "flex", alignItems: "center", gap: 6, background: "var(--warning-bg)", border: "1px solid rgba(245,158,11,0.25)", borderRadius: "var(--radius-sm)", padding: "4px 10px", color: "var(--warning)", fontSize: "0.75rem", fontWeight: 600 }}>
