...
const API          = 'http://localhost:3000/finflow/api';
...
// Estado global simples sem Context (mantém padrão do arquivo único)
let _authListeners = [];
...
function setAuthState(next) {
  _authState = next;
  _authListeners.forEach(fn => fn(next));
}
...
function LoginPage({ toast }) {
  ...
  const [dark, setDark]       = useState(() => localStorage.getItem('finflow-theme') === 'dark');

  useEffect(() => {
    document.documentElement.setAttribute('data-theme', dark ? 'dark' : 'light');
    localStorage.setItem('finflow-theme', dark ? 'dark' : 'light');
  }, [dark]);
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const hasAccess = user.role === 'SUPERADMIN' || user.permissions?.includes('FINFLOW');
      if (!hasAccess) { toast('Você não tem acesso ao FinFlow', 'error'); return; }
    ...
    <div className="login-page">
      ...
      <div className="login-card">
        <div className="login-logo">
          ...
          <div>
            <div className="login-logo-name">FinFlow</div>
            <div className="login-logo-sub">Regra 50/30/20</div>
          </div>
        </div>
...
function TransactionForm({ year, month, onSave, onClose, toast }) {
  const [form, setForm] = useState({
    description: '', amount: '', payment_method: 'pix', category: 'essential',
    due_date: today(), paid: false, is_installment: false, installment_count: 2, first_payment_date: today(),
  });
  ...
  return (
    <div className="form-grid">
      <div className="form-field form-field--full">
        ...
        <input type="text" value={form.description} onChange={e => set('description', e.target.value)} placeholder="Ex: Aluguel, Academia, Netflix..." autoFocus />
      </div>
      <div className="form-field">
        <label>Valor (R$)</label>
      ...
      <div className="form-field">
        ...
        <div className="toggle-group">
          <button className={`toggle-btn ${!form.is_installment ? 'active' : ''}`} onClick={() => set('is_installment', false)}>
            <i className="bx bx-credit-card" /> À Vista
          </button>
          <button className={`toggle-btn ${form.is_installment ? 'active' : ''}`} onClick={() => set('is_installment', true)}>
      ...
      {!form.is_installment ? (
        ...
        <>
          <div className="form-field">
            ...
            <input type="number" min="2" max="48" value={form.installment_count} onChange={e => set('installment_count', e.target.value)} />
          </div>
          <div className="form-field">
...
function EditTransactionModal({ tx, onSave, onClose, toast }) {
  const [form, setForm] = useState({
    description: tx.description, amount: tx.amount, payment_method: tx.payment_method,
    category: tx.category, due_date: tx.due_date || today(), paid: tx.paid === 1,
  });
  const [saving, setSaving] = useState(false);
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));
  ...
  async function handleSave() {
    setSaving(true);
    try {
      await api.updateTransaction(tx.id, { ...form, amount: parseFloat(form.amount) });
      toast('Transação atualizada!'); onSave(); onClose();
    } catch (e) { toast(e.message, 'error'); }
    finally { setSaving(false); }
  }
...
function BarChartComparison({ data, darkMode }) {
    ...
    datasets: [
      {
        ...
        backgroundColor: 'rgba(22, 163, 74, 0.25)', borderColor: '#16a34a', borderWidth: 2, borderRadius: 6,
      },
      {
        label: 'Realizado',
        data: [realized.essential || 0, realized.personal || 0, realized.savings || 0],
        backgroundColor: ['rgba(22,163,74,0.8)', 'rgba(245,158,11,0.8)', 'rgba(249,115,22,0.8)'],
        borderColor: ['#16a34a', '#f59e0b', '#f97316'], borderWidth: 2, borderRadius: 6,
      },
    ],
  };
  ...
  const options = {
    ...
    plugins: {
      legend: { labels: { color: textColor, font: { family: 'DM Sans', size: 12 }, padding: 16 } },
...
function PercentEditor({ config, year, month, onSaved, toast }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      toast('Configurações salvas!'); onSaved();
    } catch (e) { toast(e.message, 'error'); }
    finally { setSaving(false); }
  }
  ...
  return (
    <div className="pct-editor">
      <div className="pct-editor__table">
        {[
          { key: 'e', label: 'Essenciais', color: '#16a34a' },
          { key: 'p', label: 'Pessoais',   color: '#f59e0b' },
          { key: 's', label: 'Economias',  color: '#f97316' },
        ].map(row => (
          <div key={row.key} className="pct-row">
            <span className="pct-row__dot" style={{ background: row.color }} />
            <span className="pct-row__label">{row.label}</span>
            <input type="number" min="0" max="100" step="1" value={pcts[row.key]}
      ...
      <div className={`pct-total ${valid ? 'valid' : 'invalid'}`}>
        Total: {total.toFixed(0)}%{!valid && <span> — deve somar 100%</span>}
      </div>
...
function TransactionTable({ transactions, onTogglePaid, onEdit, onDelete, toast }) {
  ...
  return (
    <div className="tx-table-wrapper">
      ...
      {filtered.length === 0 ? (
        <div className="tx-empty"><i className="bx bx-receipt" /><p>Nenhuma transação encontrada</p></div>
      ) : (
        <table className="tx-table">
          <thead>
            <tr><th>Pago</th><th>Descrição</th><th>Categoria</th><th>Valor</th><th>Vencimento</th><th>Forma</th><th>Ações</th></tr>
          </thead>
          <tbody>
            {filtered.map(tx => {
                ...
                <tr key={tx.id} className={`tx-row ${tx.paid ? 'tx-row--paid' : ''} ${overdue ? 'tx-row--overdue' : ''}`}>
                  <td>
                    <label className="table-checkbox" title={tx.paid ? 'Marcar como pendente' : 'Marcar como pago'}>
                      <input type="checkbox" checked={!!tx.paid} onChange={() => onTogglePaid(tx.id)} />
                      <span className="table-checkbox__box" />
                    </label>
                  </td>
                  <td className="tx-desc">
                    <span>{tx.description}</span>
                    {tx.is_installment && <span className="badge badge--installment">{tx.installment_number}/{tx.installment_total}</span>}
                  </td>
                  <td>
                  ...
                  <td className={`tx-amount ${tx.paid ? 'tx-amount--paid' : ''}`}>{fmt(tx.amount)}</td>
                  <td>
                  ...
                  <td className="tx-actions">
                    <button className="icon-btn" onClick={() => onEdit(tx)} title="Editar"><i className="bx bx-edit" /></button>
                    <button className="icon-btn icon-btn--danger" onClick={() => confirmDelete(tx)} title="Excluir"><i className="bx bx-trash" /></button>
                  </td>
                </tr>
              );
            })}
...
function IncomeForm({ year, month, onSave, onClose, toast }) {
  ...
  async function handleSubmit() {
    ...
    setSaving(true);
    try {
      await api.createIncome({ year, month, description: form.description.trim(), amount: parseFloat(form.amount), received: form.received });
      toast('Entrada registrada!'); onSave(); onClose();
    } catch (e) { toast(e.message, 'error'); }
    finally { setSaving(false); }
  }
  ...
  return (
    <div className="form-grid">
      <div className="form-field form-field--full">
        <label>Descrição</label>
        <input type="text" value={form.description} onChange={e => set('description', e.target.value)} placeholder="Ex: Salário, Freelance..." autoFocus />
      </div>
      <div className="form-field form-field--full">
        <label>Valor (R$)</label>
        <input type="number" min="0.01" step="0.01" value={form.amount} onChange={e => set('amount', e.target.value)} placeholder="0,00" />
...
function EditIncomeModal({ entry, onSave, onClose, toast }) {
  const [form, setForm] = useState({ description: entry.description, amount: entry.amount, received: entry.received === 1 });
  const [saving, setSaving] = useState(false);
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await api.updateIncome(entry.id, { description: form.description.trim(), amount: parseFloat(form.amount), received: form.received });
      toast('Entrada atualizada!'); onSave(); onClose();
    } catch (e) { toast(e.message, 'error'); }
    finally { setSaving(false); }
  }
  ...
  return (
    <div className="form-grid">
      <div className="form-field form-field--full">
        <label>Descrição</label>
        <input type="text" value={form.description} onChange={e => set('description', e.target.value)} autoFocus />
      </div>
      <div className="form-field form-field--full">
        <label>Valor (R$)</label>
        <input type="number" min="0.01" step="0.01" value={form.amount} onChange={e => set('amount', e.target.value)} />
...
function IncomeTable({ incomes, onToggleReceived, onEdit, onDelete, toast }) {
  ...
  async function handleDelete(id) {
    try { await api.deleteIncome(id); toast('Entrada removida!'); onDelete(); }
    catch (e) { toast(e.message, 'error'); }
  }
    ...
    <div className="income-table-wrapper">
      <div className="income-totals">
        <div className="income-total income-total--received">
          <i className="bx bx-check-circle" /><div><span>Recebido</span><strong>{fmt(totalReceived)}</strong></div>
        </div>
        <div className="income-total income-total--pending">
          <i className="bx bx-time" /><div><span>A Receber</span><strong>{fmt(totalPending)}</strong></div>
        </div>
        <div className="income-total income-total--total">
          <i className="bx bx-wallet" /><div><span>Total Previsto</span><strong>{fmt(totalReceived + totalPending)}</strong></div>
        </div>
      </div>
      {incomes.length === 0 ? (
        <div className="tx-empty"><i className="bx bx-dollar-circle" /><p>Nenhuma entrada registrada neste mês</p></div>
      ) : (
        <table className="tx-table">
          <thead><tr><th>Recebido</th><th>Descrição</th><th>Valor</th><th>Ações</th></tr></thead>
          <tbody>
            {incomes.map(entry => (
              <tr key={entry.id} className={`tx-row ${entry.received ? 'tx-row--paid' : ''}`}>
                <td>
                  <label className="table-checkbox">
                    <input type="checkbox" checked={!!entry.received} onChange={() => onToggleReceived(entry.id)} />
                    <span className="table-checkbox__box" />
                  </label>
                </td>
                <td className="tx-desc"><span>{entry.description}</span></td>
                <td className={`tx-amount ${entry.received ? 'tx-amount--received' : ''}`}>{fmt(entry.amount)}</td>
                <td className="tx-actions">
                  <button className="icon-btn" onClick={() => onEdit(entry)} title="Editar"><i className="bx bx-edit" /></button>
                  <button className="icon-btn icon-btn--danger" onClick={() => handleDelete(entry.id)} title="Excluir"><i className="bx bx-trash" /></button>
                </td>
              </tr>
            ))}
          </tbody>
...
function UpcomingPanel({ onTogglePaid }) {
  ...
  const load = useCallback(async () => {
    try { const data = await api.getUpcoming(); setItems(data); }
  ...
  const handleToggle = async (id) => { await api.togglePaid(id); load(); onTogglePaid?.(); };

  return (
    <div className="upcoming-panel">
      <h3 className="panel-title"><i className="bx bx-calendar-event" /> Contas a Vencer (30 dias)</h3>
      {loading ? (
        <div className="loader"><i className="bx bx-loader-alt bx-spin" /></div>
      ) : items.length === 0 ? (
        <div className="upcoming-empty"><i className="bx bx-check-shield" /><span>Nenhuma conta pendente nos próximos 30 dias</span></div>
      ) : (
...
const newGoalId = () => `goal_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`;
...
function GoalDonut({ goal, darkMode }) {
  ...
  const chartData = {
    ...
    datasets: [{
      data: target > 0 ? [Math.min(totalDeposited, target), remaining] : [0, 1],
  ...
  return (
    <div className="goal-donut-wrapper">
      <div className="goal-donut-chart">
        <Doughnut data={chartData} options={options} />
        <div className="goal-donut-center"><span className="goal-donut-pct" style={{ color }}>{pct}%</span></div>
      </div>
      <div className="goal-donut-info">
        <div className="goal-donut-row"><span className="goal-donut-label">Alcançado</span><span className="goal-donut-val" style={{ color }}>{fmt(totalDeposited)}</span></div>
        <div className="goal-donut-row"><span className="goal-donut-label">Falta</span><span className="goal-donut-val goal-donut-val--muted">{fmt(remaining)}</span></div>
        <div className="goal-donut-row"><span className="goal-donut-label">Meta</span><span className="goal-donut-val goal-donut-val--target">{fmt(target)}</span></div>
      </div>
    </div>
  );
}
...
function GoalBlock({ goal, darkMode, onUpdate, onDelete, toast }) {
  ...
  function handleSaveMeta() {
    ...
    onUpdate({ ...goal, name: metaForm.name.trim(), target: parseFloat(metaForm.target), colorIdx: metaForm.colorIdx, color: GOAL_COLORS[metaForm.colorIdx].bg });
    setEditingMeta(false); toast('Meta atualizada!');
  }
  ...
  function handleAddDeposit() {
    ...
    const deposit = { id: `dep_${Date.now()}`, date: depositForm.date, amount: parseFloat(depositForm.amount) };
    onUpdate({ ...goal, deposits: [...(goal.deposits || []), deposit] });
    setDepositForm({ date: today(), amount: '' }); setAddingDeposit(false); toast('Aporte registrado!');
  }
    ...
    <div className="goal-block" style={{ '--goal-color': color.bg, '--goal-light': color.light }}>
      <div className="goal-block__header">
        <div className="goal-block__title-area">
          ...
          {editingMeta
            ? <input className="goal-block__name-input" value={metaForm.name} onChange={e => setMetaForm(f => ({ ...f, name: e.target.value }))} autoFocus />
            : <h3 className="goal-block__name">{goal.name}</h3>}
        </div>
        <div className="goal-block__actions">
          {editingMeta ? (
            <>
              <button className="icon-btn icon-btn--save" onClick={handleSaveMeta}><i className="bx bx-check" /></button>
              <button className="icon-btn" onClick={() => { setEditingMeta(false); setMetaForm({ name: goal.name, target: goal.target, colorIdx: goal.colorIdx ?? 0 }); }}><i className="bx bx-x" /></button>
            </>
          ) : (
            <>
              <button className="icon-btn" onClick={() => setEditingMeta(true)} title="Editar meta"><i className="bx bx-edit" /></button>
              <button className="icon-btn icon-btn--danger" onClick={() => { if (window.confirm(`Excluir a meta "${goal.name}"?`)) onDelete(goal.id); }} title="Excluir"><i className="bx bx-trash" /></button>
            </>
          )}
        </div>
      </div>
      ...
      {editingMeta && (
        <div className="goal-edit-form">
          <div className="form-field">
            ...
            <input type="number" min="0.01" step="0.01" value={metaForm.target} onChange={e => setMetaForm(f => ({ ...f, target: e.target.value }))} />
          </div>
          <div className="form-field">
            <label>Cor do Bloco</label>
            <div className="goal-color-picker">
              {GOAL_COLORS.map((c, i) => (
                <button key={i} className={`goal-color-swatch ${metaForm.colorIdx === i ? 'active' : ''}`}
                  style={{ background: c.bg }} onClick={() => setMetaForm(f => ({ ...f, colorIdx: i }))} title={c.name} />
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="goal-deposits">
        <div className="goal-deposits__header">
          <span className="goal-deposits__title"><i className="bx bx-calendar-plus" /> Aportes</span>
          <button className="btn btn--sm goal-add-deposit-btn" style={{ '--btn-color': color.bg }} onClick={() => setAddingDeposit(v => !v)}>
        ...
        {addingDeposit && (
          <div className="goal-deposit-form">
            ...
            <button className="btn btn--primary btn--sm" style={{ background: color.bg, borderColor: color.bg }} onClick={handleAddDeposit}>
        ...
        {deposits.length === 0 ? (
          <div className="goal-no-deposits"><i className="bx bx-coin-stack" /><span>Nenhum aporte registrado ainda</span></div>
        ) : (
          <div className="goal-deposit-list">
            {deposits.map(dep => (
              <div key={dep.id} className="goal-deposit-item">
                <span className="goal-deposit-date"><i className="bx bx-calendar" />{fmtDate(dep.date)}</span>
                <span className="goal-deposit-amount" style={{ color: color.bg }}>{fmt(dep.amount)}</span>
                <button className="icon-btn icon-btn--danger icon-btn--xs" onClick={() => {
                  onUpdate({ ...goal, deposits: (goal.deposits || []).filter(d => d.id !== dep.id) });
                  toast('Aporte removido!');
                }} title="Remover aporte"><i className="bx bx-x" /></button>
              </div>
            ))}
          </div>
        )}
...
function GoalsView({ darkMode, toast }) {
  ...
  function persist(updated) { setGoals(updated); saveGoals(updated); }
  ...
  function handleCreate() {
    ...
    persist([...goals, { id: newGoalId(), name: newForm.name.trim(), target: parseFloat(newForm.target), colorIdx: newForm.colorIdx, color: GOAL_COLORS[newForm.colorIdx].bg, deposits: [], createdAt: new Date().toISOString() }]);
    setNewForm({ name: '', target: '', colorIdx: 0 }); setShowNewForm(false); toast('Meta criada!');
  }
  ...
  return (
    <div className="goals-view">
      <div className="goals-header">
        <div>
          <h2 className="goals-title"><i className="bx bx-target-lock" /> Metas &amp; Objetivos</h2>
          <p className="goals-subtitle">Acompanhe seus objetivos financeiros e registre seus aportes.</p>
        </div>
        <button className="btn btn--primary" onClick={() => setShowNewForm(v => !v)} disabled={goals.length >= 10}>
        ...
        <div className="goal-new-form card">
          ...
          <div className="form-grid">
            <div className="form-field form-field--full">
              ...
              <input type="text" value={newForm.name} onChange={e => setNewForm(f => ({ ...f, name: e.target.value }))} placeholder="Ex: Viagem Europa, Fundo de emergência..." autoFocus />
            </div>
            <div className="form-field">
              <label>Valor da Meta (R$)</label>
              <input type="number" min="0.01" step="0.01" value={newForm.target} onChange={e => setNewForm(f => ({ ...f, target: e.target.value }))} placeholder="0,00" />
            </div>
            <div className="form-field">
              <label>Cor do Bloco</label>
              <div className="goal-color-picker">
                {GOAL_COLORS.map((c, i) => (
                  <button key={i} className={`goal-color-swatch ${newForm.colorIdx === i ? 'active' : ''}`}
                    style={{ background: c.bg }} onClick={() => setNewForm(f => ({ ...f, colorIdx: i }))} title={c.name} />
                ))}
              </div>
            </div>
            <div className="form-actions form-field--full">
              <button className="btn btn--ghost" onClick={() => setShowNewForm(false)}>Cancelar</button>
              <button className="btn btn--primary" onClick={handleCreate}><i className="bx bx-plus" /> Criar Meta</button>
            </div>
          </div>
        </div>
      )}
      ...
      {goals.length === 0 ? (
        ...
        <div className="goals-grid">
          {goals.map(goal => (
            <GoalBlock key={goal.id} goal={goal} darkMode={darkMode}
              onUpdate={updated => persist(goals.map(g => g.id === updated.id ? updated : g))}
              onDelete={id => { persist(goals.filter(g => g.id !== id)); toast('Meta removida!'); }}
              toast={toast} />
          ))}
        </div>
      )}
...
function MonthView({ year, month, darkMode, toast }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const d = await api.getMonth(year, month); setData(d); }
    catch (e) { toast(e.message, 'error'); }
    finally { setLoading(false); }
  }, [year, month]);
  ...
  if (loading) return <div className="month-loader"><i className="bx bx-loader-alt bx-spin" /><span>Carregando...</span></div>;
  if (!data) return null;
    ...
    <div className="month-view">
      <div className="summary-cards">
        <div className="summary-card summary-card--income">
          ...
          <div>
            ...
            {totalIncomeReceived > 0 && <span className="summary-card__sub">+ {fmt(totalIncomeReceived)} entradas</span>}
          </div>
        </div>
        <div className="summary-card summary-card--spent">
          <i className="bx bx-trending-down" /><div><span>Gasto (pago)</span><strong>{fmt(totalRealized)}</strong></div>
        </div>
        <div className="summary-card summary-card--pending">
          <i className="bx bx-time" /><div><span>Pendente</span><strong>{fmt(totalPending)}</strong></div>
        </div>
        <div className={`summary-card ${balance >= 0 ? 'summary-card--balance' : 'summary-card--negative'}`}>
          ...
          <div><span>Saldo</span><strong>{fmt(balance)}</strong></div>
        </div>
      </div>
      ...
      {activeTab === 'overview' && (
        <div className="overview-grid">
          <div className="card">
            <h3 className="card__title"><i className="bx bx-pie-chart-alt-2" /> Distribuição Real</h3>
            <DonutChart data={data} income={income} darkMode={darkMode} />
          </div>
          <div className="card">
            <h3 className="card__title"><i className="bx bx-bar-chart-alt" /> Planejado vs Realizado</h3>
                ...
                return (
                  <div key={key} className="planned-item">
                    ...
                    <div className="progress-bar">
                      <div className={`progress-bar__fill ${over ? 'progress-bar__fill--over' : ''}`} style={{ width: `${overPct}%`, background: cat.color }} />
                    </div>
                  </div>
                );
              })}
      ...
      {activeTab === 'incomes' && (
        <div className="card">
          <div className="card__header">
            <h3 className="card__title"><i className="bx bx-dollar-circle" /> Entradas do Mês</h3>
            <button className="btn btn--income btn--sm" onClick={() => setShowAddIncome(true)}><i className="bx bx-plus" /> Nova Entrada</button>
          </div>
          <IncomeTable incomes={incomes} onToggleReceived={handleToggleReceived} onEdit={setEditIncome} onDelete={load} toast={toast} />
        </div>
      )}
      ...
      {activeTab === 'transactions' && (
        <div className="card">
          <div className="card__header">
            <h3 className="card__title"><i className="bx bx-list-ul" /> Gastos do Mês</h3>
            <button className="btn btn--primary btn--sm" onClick={() => setShowAdd(true)}><i className="bx bx-plus" /> Adicionar</button>
          </div>
          <TransactionTable transactions={transactions} onTogglePaid={handleTogglePaid} onEdit={setEditTx} onDelete={load} toast={toast} />
        </div>
      )}

      {activeTab === 'settings' && (
        <div className="card card--narrow">
          <h3 className="card__title"><i className="bx bx-slider" /> Configurar Mês</h3>
          <p className="card__subtitle">Defina os percentuais da regra 50/30/20 para distribuição dos gastos.</p>
          <PercentEditor config={config} year={year} month={month} onSaved={load} toast={toast} />
        </div>
      )}

      {showAdd && <Modal title="Nova Transação" onClose={() => setShowAdd(false)} size="md"><TransactionForm year={year} month={month} onSave={load} onClose={() => setShowAdd(false)} toast={toast} /></Modal>}
      {editTx && <Modal title="Editar Transação" onClose={() => setEditTx(null)} size="md"><EditTransactionModal tx={editTx} onSave={load} onClose={() => setEditTx(null)} toast={toast} /></Modal>}
      {showAddIncome && <Modal title="Nova Entrada" onClose={() => setShowAddIncome(false)} size="sm"><IncomeForm year={year} month={month} onSave={load} onClose={() => setShowAddIncome(false)} toast={toast} /></Modal>}
      {editIncome && <Modal title="Editar Entrada" onClose={() => setEditIncome(null)} size="sm"><EditIncomeModal entry={editIncome} onSave={load} onClose={() => setEditIncome(null)} toast={toast} /></Modal>}
    </div>
  );
}
...
function Sidebar({ year, month, onSelect, onYearChange, darkMode, onToggleDark, activePage, onPageChange, toast }) {
  const { user } = useAuthState();

  function handleLogout() { doLogout(); toast('Sessão encerrada', 'info'); }

  return (
    <aside className="sidebar">
      <div className="sidebar__brand">
        <i className="bx bx-leaf" /><span>FinFlow</span>
      </div>

      <div className="sidebar__nav-top">
        <button className={`sidebar-nav-btn ${activePage === 'goals' ? 'sidebar-nav-btn--active' : ''}`} onClick={() => onPageChange('goals')}>
          <i className="bx bx-target-lock" /><span>Metas</span>
          {activePage === 'goals' && <i className="bx bx-chevron-right sidebar-nav-btn__arrow" />}
        </button>
      </div>

      <div className="sidebar__year">
        <button className="year-nav" onClick={() => onYearChange(year - 1)}><i className="bx bx-chevron-left" /></button>
        <span className="year-label">{year}</span>
        <button className="year-nav" onClick={() => onYearChange(year + 1)}><i className="bx bx-chevron-right" /></button>
      </div>
      ...
      <nav className="sidebar__months">
        {MONTHS_PT.map((name, i) => {
          ...
          return (
            <button key={m} className={`month-btn ${isActive ? 'month-btn--active' : ''}`}
...
export default function App() {
  ...
  return (
    <div className="app">
      <Sidebar
        year={year} month={month} onSelect={setMonth} onYearChange={setYear}
        darkMode={darkMode} onToggleDark={() => setDarkMode(d => !d)}
        activePage={activePage} onPageChange={setActivePage} toast={toast}
      />
      ...
      <main className="main">
        <header className="topbar">
          <div className="topbar__title">
            {isGoalsPage ? (
              <><h1>Metas <span>&amp; Objetivos</span></h1><p className="topbar__rule">Acompanhe e registre seus objetivos financeiros</p></>
            ) : (
              <><h1>{MONTHS_PT[month - 1]} <span>{year}</span></h1><p className="topbar__rule">Regra 50/30/20 — Controle Financeiro Pessoal</p></>
            )}
          </div>
          {!isGoalsPage && (
            <div className="topbar__actions">
              <button className="btn btn--ghost btn--sm" onClick={() => setShowUpcoming(v => !v)}>
                <i className="bx bx-calendar-check" /> Próximos Vencimentos
              </button>
            </div>
          )}
        </header>

        {!isGoalsPage && showUpcoming && <UpcomingPanel onTogglePaid={() => {}} />}

        {isGoalsPage
          ? <GoalsView darkMode={darkMode} toast={toast} />
          : <MonthView key={`${year}-${month}`} year={year} month={month} darkMode={darkMode} toast={toast} />
        }
      </main>
